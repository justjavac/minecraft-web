...
- 自定义贴图包两种方式（自动识别 Faithful 等现代 MC 包结构，16/32px 自适应）：
  - 机器级默认：`bash scripts/install-texture-pack.sh <贴图包目录>` 安装到 `public/textures/pack/`（已 gitignore，不入库不分发）
  - 浏览器级覆盖：设置里导入 zip 或解压文件夹（存 localStorage），优先级高于本地安装包
...
scripts/                — fetch-textures.sh / fetch-sounds.sh / install-texture-pack.sh / generate-icons.mjs
```
