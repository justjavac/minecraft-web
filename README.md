...
- **下界维度**：黑曜石 4×5 门框（角可选）+ 打火石（砂砾 10% 掉燧石）点燃传送门，门内站 3 秒传送（坐标 1:8 映射，MC 规则）；下界为下界岩丘陵 + y31 岩浆海 + 参差基岩顶 + 石英矿/萤石簇/灵魂沙；**僵尸猪灵**成群出没——中立怪，打一只则群体仇恨；两个维度独立存档、随时往返
...
- 脚本：`pnpm dev` / `pnpm test`（vitest 230 用例）/ `pnpm lint` / `pnpm build`（静态导出 `out/`，离线可构建，可部署到任意静态托管）
