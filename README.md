...
- **红石基础**：红石矿掉红石粉，合成红石火把（常亮电源）/拉杆（右击开关）/红石块/红石灯；红石粉贴地传能（邻接电源 15 级、逐格衰减）；元件反应：**红石灯亮灭、橡木门供能自动开关、TNT 供能引爆、活塞推收**——普通/粘性**活塞**（6 朝向、最多推 12 格、黑曜石基岩容器不可推、粘性断供拉回，MC 规则）
...
- 脚本：`pnpm dev` / `pnpm test`（vitest 281 用例）/ `pnpm lint` / `pnpm build`（静态导出 `out/`，离线可构建，可部署到任意静态托管）
