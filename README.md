...
- **结构**：平原/热带草原/针叶林/雪原/沙漠村庄（材质随群系：橡木/金合欢/云杉/砂岩）、哨塔、冰屋、**沙漠神殿**（阶梯金字塔 + 密藏室 4 宝箱 + TNT 陷阱层）、**丛林神庙**（苔石殿 + 明暗双宝箱，战利品含竹子）、**废弃传送门**（下界岩 + 残破黑曜石门框 + 金宝箱），白天村庄附近有村民
...
- 脚本：`pnpm dev` / `pnpm test`（vitest 203 用例）/ `pnpm lint` / `pnpm build`（静态导出 `out/`，离线可构建，可部署到任意静态托管）
