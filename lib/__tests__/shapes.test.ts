...
describe('形状碰撞盒', () => {
  it('台阶底 [0,0.5] / 台阶顶 [0.5,1] / 栅栏 [0,1.5] / 花草无碰撞', () => {
    expect(blockBox(BLOCK_BY_KEY.stone_slab.id)).toEqual([0, 0.5]);
    expect(blockBox(BLOCK_BY_KEY.stone_slab_top.id)).toEqual([0.5, 1]);
    expect(blockBox(BLOCK_BY_KEY.oak_fence.id)).toEqual([0, 1.5]);
    expect(blockBox(BLOCK_BY_KEY.dandelion.id)).toBeNull();
    expect(blockBox(STONE)).toEqual([0, 1]);
  });
